...
REQUIRED BEHAVIOR
- Generate exactly 13 tags; each tag MUST be 20 characters or fewer (hard limit). If a tag would exceed 20 characters, rephrase it as a shorter real phrase — NEVER cut off or abbreviate a word, which produces broken/misspelled tags. Spell every tag correctly. Theme-first relevancy; mix head terms + modifiers.
...
### Tags
tag1, tag2, … (exactly 13; **each tag MUST be 20 characters max** — hard limit; if too long, rephrase shorter, **never cut off or abbreviate a word**; **spell every tag correctly**; no duplicates; **no repeated words across tags**; mix of head + modifiers; **avoid generic standalone terms** — always combine the core product subject with modifiers for specificity)
...
Rules:
- Each tag MUST be 20 characters or fewer (hard limit). If a tag would exceed 20 characters, rephrase it shorter — NEVER cut off or abbreviate a word. Spell every tag correctly; no abbreviations.
